...
/*                                                        */
/* Copyright 2016 by Dieter Lambrecht for changes to work with Conrad Duino Mega32 board */
/*											*/
...
/* LED_ON_BOOTLOADER				*/
/* Switch on the LED when Bootloader is running	-dl	*/
/*                                                        */
/* LED_START_FLASHES:                                     */
...
// Conrad Duino Mega32 board
#define _DUINO_MEGA32_
#define F_CPU 14746000L
#define BAUD_RATE 19200
#define LED_ON_BOOTLOADER
...
#if defined(BIGBOOT)
// Bootloader 1024 (high=0xDA, low=0xFF)
__fuse_t __fuse __attribute__((section (".fuse"))) =
{
	.low = 0xFF,
	.high = (FUSE_BOOTSZ1 & FUSE_BOOTRST & FUSE_SPIEN)
};
#else
// Bootloader 512 (high=0xDC, low=0xFF)
__fuse_t __fuse __attribute__((section (".fuse"))) =
{
	.low = 0xFF,
	.high = (FUSE_BOOTSZ0 & FUSE_BOOTRST & FUSE_SPIEN)
};
#endif
...
#if LED_START_FLASHES > 0					// saves some spaces if  LED_START_FLASHES > 0 -dl
...
int main(void) {
...
#if defined (_DUINO_MEGA32_)
 // port settings for bootloader button 
 BLDDR  = 0b11111011;   // set PD2 to input
 BLPORT = 0b00000100;   // set PD2 pullup
#endif
...
#if defined (_DUINO_MEGA32_)
	// if Bootloader Button not pressed - start the app directly, other wise stay in bootloader and wait for commands -dl
	if (PIND & (1<<PD2)) {
		appStart(ch);
	}else {
		// start the app after powerup
 ...
 #if !defined (_DUINO_MEGA32_)
	// no watchdog for Duino Mega32 - Bootloader button is used - dl
    // Set up watchdog to trigger after 1s
	watchdogConfig(WATCHDOG_1S);
#endif

#if (LED_START_FLASHES > 0) || defined(LED_DATA_FLASH) || defined(LED_ON_BOOTLOADER) //-dl
...
#if LED_START_FLASHES > 0
#if defined (LED_ON_BOOTLOADER)		//-dl
#error Please set LED_START_FLASHES to 0 OR undefine LED_ON_BOOTLOADER
#endif
